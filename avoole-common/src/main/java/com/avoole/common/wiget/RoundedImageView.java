...
import com.avoole.common.R.styleable;
...
public class RoundedImageView extends AppCompatImageView {
    ...
    public RoundedImageView(Context context, AttributeSet attrs, int defStyle) {
        ...
        float cornerRadiusOverride = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_corner_radius, -1);
        this.mCornerRadii[0] = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_corner_radius_top_left, -1);
        this.mCornerRadii[1] = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_corner_radius_top_right, -1);
        this.mCornerRadii[2] = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_corner_radius_bottom_right, -1);
        this.mCornerRadii[3] = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_corner_radius_bottom_left, -1);
        boolean any = false;
        int tileMode = 0;
        ...
        this.mBorderWidth = (float)a.getDimensionPixelSize(styleable.RoundedImageView_av_border_width, -1);
        if (this.mBorderWidth < 0.0F) {
            this.mBorderWidth = 0.0F;
        }

        this.mBorderColor = a.getColorStateList(styleable.RoundedImageView_av_border_color);
        if (this.mBorderColor == null) {
            this.mBorderColor = ColorStateList.valueOf(-16777216);
        }

        this.mMutateBackground = a.getBoolean(styleable.RoundedImageView_av_mutate_background, false);
        this.mIsOval = a.getBoolean(styleable.RoundedImageView_av_oval, false);
        tileMode = a.getInt(styleable.RoundedImageView_av_tile_mode, -2);
        if (tileMode != -2) {
            this.setTileModeX(parseTileMode(tileMode));
            this.setTileModeY(parseTileMode(tileMode));
        }

        tileModeX = a.getInt(styleable.RoundedImageView_av_tile_mode_x, -2);
        if (tileModeX != -2) {
            this.setTileModeX(parseTileMode(tileModeX));
        }

        int tileModeY = a.getInt(styleable.RoundedImageView_av_tile_mode_y, -2);
        if (tileModeY != -2) {
